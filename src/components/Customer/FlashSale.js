...
const FlashSale = () => {
    ...
    // Tách riêng: khung giờ đang diễn ra (không phụ thuộc selected)
    ...
    const formatTimeButtonContent = (slot) => {
        ...
        return (
            <div className={styles.slotInner}>
                {/* Nhãn trạng thái */}
                {isActive && (
                    <div className={`${styles.slotLabel} ${styles.activeLabel}`}>Đang diễn ra</div>
                )}
                {!isActive && isFuture && !isTomorrow && (
                    <div className={styles.slotLabel}>Sắp diễn ra</div>
                )}
                {!isActive && !isFuture && !isTomorrow && (
                    <div className={styles.slotLabelDisabled}>Đã kết thúc</div>
                )}
                {isTomorrow && (
                    <div className={styles.slotLabel}>Ngày mai</div>
                )}
                ...
                {/* Đếm ngược chỉ hiện trong nút đang active */}
                {isActive && (
                            ...
                            {(flashSaleData.today || []).map(slot => (
                                <div
                                    ...
                                    className={`${styles.timeSlotButton} ${
                                        selectedTimeSlot?.flashSaleId === slot.flashSaleId ? styles.active : ''
                                    }`}
